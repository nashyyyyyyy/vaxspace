...
def add_record(request, id):
    ...
    if request.method == 'POST':
        ...
        if Schedule.objects.filter(child = id).exists():
            ...
            save_date = Schedule(child=register_id,vaccine_info=vaccine_info,vax_date=vaccination_date_with_timezone, brgy=barangay_id, guardian=guardian_id, created=created)
            save_date.save()
            messages.success(request, "Added successfully.")
            return redirect('tables/..')
